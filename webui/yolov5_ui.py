...
class YOLOv5Detector:
    ...
    def detect_objects_ss(self, img, conf, iou, line_width, device, model_type, model_path):
        if model_type == "yolov5n":
            self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='../weights/yolov5/yolov5n.pt')
            ...
            self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='../weights/yolov5/yolov5m.pt')
        elif model_type == "yolov5l":
            self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='../weights/yolov5/yolov5l.pt')
        elif model_type == "yolov5x":
            self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='../weights/yolov5/yolov5x.pt')
        ...
        for bbox in xyxy.itertuples():
            ...
            if confidence > conf:
                # Draw bounding box
                cv2.rectangle(img, (xmin, ymin), (xmax, ymax), color, line_width)
                # Draw label name
                label = f"{name}: {confidence:.2f}"
                cv2.putText(img, label, (xmin, ymin - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, line_width)
    ...
    iface = gr.Interface(
        ...
        inputs=["image",
                ...
                gr.inputs.Slider(minimum=0, maximum=1, step=0.01, default=0.45,
                                 label="IoU Threshold"),
                gr.inputs.Number(default=3, label="Line Width"),
                gr.inputs.Radio(["cpu", "cuda"], label="Device", default="cpu"),
                gr.inputs.Radio(["yolov5s", "yolov5m", "yolov5l", "yolov5x"],
